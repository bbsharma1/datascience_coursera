...
plot4data <- consump[consump$Date %in% c("1/2/2007","2/2/2007"), ]

plot4data$Date_Time <- as.POSIXct(strptime(paste(plot4data$Date, plot4data$Time), format = "%d/%m/%Y %H:%M:%S"))

# New data without the seperate Date and Time
plot4data <- plot4data[, c("Date_Time", "Global_active_power", "Global_reactive_power", "Voltage", 
...
with(plot4data, {
  ...
  plot(Voltage ~ Date_Time, type = "l", col = "black", xlab="datetime")
  ...
  plot(Global_reactive_power ~ Date_Time, type = "l", col = "black", xlab="datetime")
})
dev.off()
## Observation: cex controls size of legend box, and bty controls border
##################################################################################################################